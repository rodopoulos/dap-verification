...
In this section, the obtained model properties so far are described. This properties relate to the protocol guarantees and it is a partial formalization. More work on confidentiality and authorization guarantees must be made, but the results are satisfying since they explore some core features in the protocol.
...
Theorem \ref{thm:tan_a_identity_by_server} demonstrates how the server correctly links the TAN and hash \(h_S\) to the correct agent, proving that it can only authorize transactions to known peers. Thus, If the servers responds someone, it must knows the pre-shared key \(K\) belonging to such agent.
